...
class Voting {
  ...
  public $votitems_db = 'wp_voting';        // Table /or file_name to store items that are voted
  public $votusers = 'votusers';   // Table /or filename that stores the users who voted in current day
  public $votusers_db = 'wp_votusers';   // Table /or filename that stores the users who voted in current day
    ...
    if(!empty($vote)) {
      ...
      if($this->voter === '') return "alert('Vote Not registered.\\nYou must be logged in to can vote')";
        ...
        if(in_array($items[0], $votstdy) || ($this->nrvot === 1 && count($votstdy) > 0)) {
          $votstdy[] = $items[0];
          setcookie("votings", implode(',', array_unique($votstdy)), strtotime('+1 month'));
  ...
  protected function setVotDb($items, $vote, $votstdy) {
    $this->sqlExecute("INSERT INTO `$this->votitems_db` (`item`, `vote`) VALUES ('".$items[0]."', $vote) ON DUPLICATE KEY UPDATE `vote`=`vote`+$vote, `nvotes`=`nvotes`+1");

    $this->sqlExecute("DELETE FROM `$this->votusers_db` WHERE `day`!=$this->tdy");

    $this->sqlExecute("INSERT INTO `$this->votusers_db` (`day`, `voter`, `item`) VALUES ($this->tdy, '$this->voter', '".$items[0]."')");
  ...
  protected function getVotDb($items, $votstdy, $setvoted) {
    ...
    $resql = $this->sqlExecute("SELECT * FROM `$this->votitems_db` WHERE `item` IN(".implode(',', array_map('addSlhs', $items)).")");
  ...
  protected function votstdyDb() {
    $votstdy = array();
    $resql = $this->sqlExecute("SELECT `item` FROM `$this->votusers_db` WHERE `day`=$this->tdy AND `voter`='$this->voter'");
    if($this->affected_rows > 0) {
      for($i=0; $i<$this->affected_rows; $i++) {
